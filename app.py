...
def invert(decode_array_to_img):
    invert_image = cv2.bitwise_not(decode_array_to_img)
    status, output_img = cv2.imencode('.PNG', invert_image)
    return output_img

def HDR(decode_array_to_img):
    hdr_image = cv2.detailEnhance(decode_array_to_img, sigma_s=12, sigma_r=0.5)
